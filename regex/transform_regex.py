...
hos_kor = "건국|충북대|양산부산|CMC|단국대|계명대|가톨릭대학교성모|계명|KUMC|KHUH|본원|성모|DAMC|서울대|고려대|고대|고대|안암|서울대학교 강남센터|DKUH|대구가톨릭대|전남|동아|이화여자|경상|KBSMS|건국대학교|고려대학교|연세대|연대|중앙대|경북|울산대학교|단국대학교의과대학부속|순천향|중앙|아주|아주대|SNUH|GNUH|서울|전북|원광대학교|충북|AMC|강북삼성|칠곡경북|CBNUH|UUH|경북대|SMC|연건|원광|보라매|대구가톨릭대학교|신촌|신촌세브란스|전북대|전남대학교|울산|충남|CNUH|이화여자대학교의과대학|울산대|삼성서울|가톨릭대|영남대|영남대학교|순천향대학교|양산부산대|YUMC|한양|KNUCH|분당서울대|경상대|구가톨릭|JBUH|강남센터|AJOUMC|순천향대|SCHMC|영남|칠곡경북대학교|양산부산대학교|분당서울대학교|충남대|계명대학교동산|동아대|가톨릭|중앙대학교|한양대학교|WKUH|경상대학교|분당서울|아산|KNUH|고려|삼성|AJOU|강남세브란스|경북대학교|부산|PNUH|이화여자대|이화여대|전북대학교|건국대|건대|충대|서울아산|SNUBH|PNUYH|전남대|연세대학교세브란스|충북대학교|단국|한양대|EUMC|경희대학교|CAUMC|KUH|부산대|HYUMC|경희|DSMC|DCMC|원광대|칠곡경북대|아주대학교|충남대학교|동아대학교|부산대학교|연세|경희대|서울대학교|SNUCH|DMC|분당차"
...
for subdir, dirs, files in os.walk(rootdir):
	for file in files:
		if ".txt" in file and "_transformed.txt" not in file and "catchall" not in file:
			...
			# Replace variables
			regex = regex.replace('"""+month_name+r"""', month_name).replace('"""+day_numbering+r"""', day_numbering).replace('"""+day_name+r"""', day_name).replace('"""+seasons+r"""', seasons).replace('"""+address_indicator+r"""',address_indicator).replace('"""+state_name+r"""', state_names).replace('"""+full_numbering+r"""', full_numbering).replace('"""+hos_kor+r"""', hos_kor)	
			# Write new file
